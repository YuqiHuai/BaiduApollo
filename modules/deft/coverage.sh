...
# A replay that aborts dumps core to /apollo/data/core (see core_pattern), which
# is the worker's Apollo tree on the host. Those cores are gigabytes each and
# nothing reads them: a failed replay is already reported through the test log
# and the record is recorded as failed. Left enabled they fill the disk, and
# every later record then fails with ENOSPC rather than on its own merits.
ulimit -c 0
